...
//构建任务耗时映射
std::unordered_map<std::string, double> LoadCostMap(const std::string& filename) {
    std::ifstream in(filename);
    if (!in.is_open()) {
        Fatal("Failed to open edge cost file: %s", filename.c_str());
    }
    ...
    for (auto it = j.begin(); it != j.end(); ++it) {
        const std::string& output_path = it.key();
        double cost = it.value();
        cost_map[output_path] = cost;
    }
...
void Plan::InjectEdgeCosts(const std::unordered_map<std::string, double>& cost_map) {
  for (const auto& pair : want_) {
    Edge* edge = pair.first;
    if (!edge || edge->outputs_.empty()) continue;

    const std::string& output_path = edge->outputs_[0]->path();

    auto it = cost_map.find(output_path);
    if (it != cost_map.end()) {
      edge->estimated_cost = it->second;

      //printf("[InjectEdgeCosts] %s -> estimated_cost = %.5f\n",
        //     output_path.c_str(), edge->estimated_cost);
...
Edge* Plan::FindWork() {
 ...
 // printf("[SCHED-RAW] Estimated cost (full double): %.5f\n", work->estimated_cost);
...
void Plan::ScheduleWork(map<Edge*, Want>::iterator want_e) {
  ...
  if (pool->ShouldDelayEdge()) {
    ...
    pool->EdgeScheduled(*edge);
     // ------------------- Debug print ----------------------
    printf("[ScheduleWork] estimated_cost = %.4f, outputs: ", edge->estimated_cost);
    for (Node* output : edge->outputs_) {
      printf("%s ", output->path().c_str());
    }
    printf("\n");
    // ------------------------------------------------------
    ready_.push(edge);
...
ExitStatus Builder::Build(string* err) {
  ...
  std::unordered_map<std::string, double> cost_map = LoadCostMap("/home/enovoleekira/function_test/project/Catch2/edge_costs/edge_costs.json");
  plan_.InjectEdgeCosts(cost_map);
  plan_.PrepareQueue();
